...
class ORMMPTTTest extends PHPUnit_Extensions_Database_TestCase {
	...
	{
		$pdo_config = Kohana::$config->load('unittest.pdo');

		Database::$default = 'unittest';

		$pdo = new PDO($pdo_config['dsn'], $pdo_config['username'], $pdo_config['password']);

		$sth = $pdo->prepare(file_get_contents(Kohana::find_file('tests', 'mptt/test_data/dataset', 'sql')));
		$sth->execute();
	...
	{
		$file = Kohana::find_file('tests', 'mptt/test_data/dataset', 'xml');
	...
	{
		$root_node = ORM::factory('mptt_test', 1);
	...
	{
		$node = ORM::factory('mptt_test', $node_id);
